...
func Start(queryServiceAddr string, serverPort int) {
	...
	if err != nil {
		logger.Fatalf("Failed to connect to Open Match, got %s", err.Error())
	}
	defer conn.Close()
	...
	if err != nil {
		logger.Fatalf("TCP net listener initialization failed for port %v, got %s", serverPort, err.Error())
	}
